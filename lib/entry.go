...
// AddIPRange 直接以 [From, To] 形式写入一段连续地址，不做「区间 -> CIDR」的展开。
//
// 为什么需要它：ipinfo、iptoasn 这类数据源是按起止 IP 给区间的，单次转储动辄上百万行。
// 若逐行把区间拆成 CIDR 再走 AddPrefix（字符串 -> 解析 -> 前缀），会有两笔额外开销：
// 一是区间拆分的逐位计算，二是 CIDR 的格式化与再解析。交给 netipx.IPSetBuilder
// 在区间层面做合并，既省掉这两步，也天然得到最紧凑的前缀集合。
func (e *Entry) AddIPRange(ipRange netipx.IPRange) error {
	if !ipRange.IsValid() {
		return ErrInvalidIPRange
	}

	from, to := ipRange.From().Unmap(), ipRange.To().Unmap()
	if from.Is4() != to.Is4() {
		return ErrInvalidIPRange
	}
	...
	case from.Is4():
		...
		e.ipv4Builder.AddRange(netipx.IPRangeFrom(from, to))
		e.ipv4Set = nil
	...
	case from.Is6():
		...
		e.ipv6Builder.AddRange(netipx.IPRangeFrom(from, to))
		e.ipv6Set = nil
